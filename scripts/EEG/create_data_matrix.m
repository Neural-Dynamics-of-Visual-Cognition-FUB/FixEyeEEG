function [data] = create_data_matrix(num_conditions, min_num_trial, data_timelock, objA, objB, individual_objects)
        ...
        data = zeros(num_conditions, min_num_trial, size(data_timelock.label,1), size(data_timelock.time,2));
        data(1,:,:,:) = 1;
        data(2,:,:,:) = 0;
       ...
       % individual_objects = unique(data_timelock.trialinfo(:,4));
        ...
        data(1,:,:,:) = datasample(data_objA.trial,min_num_trial, 'Replace', false);
        data(2,:,:,:) = datasample(data_objB.trial,min_num_trial, 'Replace', false);

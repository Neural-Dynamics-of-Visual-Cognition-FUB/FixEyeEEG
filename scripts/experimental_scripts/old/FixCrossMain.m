...
cfg.blocks = 20;
cfg.trial = 150;
cfg.alltrials = cfg.blocks*cfg.trial;
cfg.distance = 60;
cfg.screensize = 54; 
n_block = 2; 
stim_pres = 0.5;
...
[images, paperclipTexture, my_images, ppd, pic_size, inanimates] = prepare_img(cfg,window);
...
[n_block,cond,dat] = randomize_conditions(cfg);

cfg.trial_order(:,1) = n_block;
...
edfFile = ['eye',num2str(data.subj_code),'.edf'];

%edfFile = 'fixcross.edf'; 
...
data.catlabel = NaN(1, cfg.alltrials);
...
for trial = 1:length(cfg.trial_order)
    ...
    if cfg.trial_order(trial,2) == 1 %with bulls eye
        if cfg.trial_order(trial,3) ~= 0  %not catch trial
            ...
            %label the category (1 for animate 0 for inanimate)
            
            if ismember(images(my_images(dat(trial))).category, inanimates) ==1
                data.catlabel(trial) = 0;
            else
                data.catlabel(trial) =1 ;
            end
    ...
    if trial == cfg.pause && trial ~= length(cfg.trial_order)
        ...
        text_pause = sprintf('You are in block %d of %d. Press any button if you want to continue.',n_block, cfg.blocks);
...
KbStrokeWait;
sca;
save(['.\FixCrossExp_s',num2str(data.subj_code),'cfg','data']);

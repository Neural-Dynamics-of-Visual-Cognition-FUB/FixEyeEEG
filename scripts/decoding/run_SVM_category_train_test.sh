...
#SBATCH --job-name=category_train_test
...
do
		for train in 1 2 
do
    permutations[$index]="$sub $methods $train" 
    index=$((index + 1))
	done
done
done 
...
matlab -nosplash -noFigureWindows -r "train_on_standard_test_on_bulls_SVM_category(${sub}, ${methods}, ${train})" > serial.out #this worked
echo set to run

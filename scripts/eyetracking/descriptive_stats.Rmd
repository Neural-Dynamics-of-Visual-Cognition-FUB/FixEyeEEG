...
ggplot(df_all_subj_all_sacs_behav_ratio, aes(y = amplitude, x=saccType, color=cross))+
         geom_violin() +
        scale_color_manual(values = c("bullseye" = rgb(146/255, 0/255, 0/255),
                                "standard"=rgb(0/255, 146/255, 146/255))) +
                     theme_classic()

ggplot(df_all_subj_all_sacs_behav_ratio, aes(y = amplitude, x=cross, color = cross,fill=cross))+
  geom_violin(aes(alpha = 0.7)) + 
  facet_grid(cols = vars(saccType))+
  scale_color_manual(values = c("bullseye" = rgb(146/255, 0/255, 0/255),
                                "standard"=rgb(0/255, 146/255, 146/255))) +
  scale_fill_manual(values = c("bullseye" = rgb(146/255, 0/255, 0/255),
                                "standard"=rgb(0/255, 146/255, 146/255))) +
...
ggplot(df_n_all_saccades, aes(y = n_occurence, x=cross, color = cross))+
         geom_count(aes(alpha = 0.7)) + facet_grid(cols = vars(saccType))+   
          scale_color_manual(values = c("bullseye" = rgb(146/255, 0/255, 0/255),
                                "standard"=rgb(0/255, 146/255, 146/255))) +
...
ggplot(df_all_subj_all_sacs_behav_ratio, aes(y = amplitude, x=cross, color = cross))+
         geom_bar(stat=) + facet_grid(cols = vars(saccType))+  
          scale_color_manual(values = c("bullseye" = rgb(146/255, 0/255, 0/255),
                                "standard"=rgb(0/255, 146/255, 146/255))) +
          theme_classic()
ggsave("/Users/ghaeberle/scratch/data/FixEyeEEG/main/results/plots/amplitudes.png")
...
ggplot(df_n_all_saccades, aes(y = n_occurence, x=cross, color = cross))+
         geom_bar(stat='identity') + facet_grid(cols = vars(saccType))+   
          scale_color_manual(values = c("bullseye" = rgb(146/255, 0/255, 0/255),
                                "standard"=rgb(0/255, 146/255, 146/255))) +
          theme_classic()
...
trials_category = ddply(df_all_subj_all_sacs_behav_ratio,c('category','subj'), summarise,
               n_trials = length(unique(trial)))
cat = aggregate(trials_category$n_trials, by=list(category=trials_category$category), FUN=sum)

sum(cat$x)/2/30
df_all_subj_all_sacs_behav_ratio$tmp = unlist(df_all_subj_all_sacs_behav_ratio$exemplar)
trials_exemplar = ddply(df_all_subj_all_sacs_behav_ratio,c('tmp','subj'), summarise,
               n_trials = length(unique(trial)))
ex = aggregate(trials_exemplar$n_trials, by=list(exemplar=trials_exemplar$tmp), FUN=sum)

sum(ex$x)/40/30
trials_category %>% subset(category=='animate') %>% sum()
sum(trials_category$n_trials[])
```
